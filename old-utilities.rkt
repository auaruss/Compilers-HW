...
(provide debug-level AST-output-syntax at-debug-level? debug verbose copious
	 ...
	 arg-registers rootstack-reg register->color color->register
         ...
         (contract-out [struct ForLoop ((var symbolic?) (seq exp?) (body exp?))])
...
(struct ForLoop (var seq body)  #:transparent #:property prop:custom-print-quotable 'never
  #:methods gen:custom-write
  [(define write-proc
     (let ([csp (make-constructor-style-printer
                 (lambda (obj) 'ForLoop)
                 (lambda (obj) (list (ForLoop-var obj) (ForLoop-seq obj)
                                     (ForLoop-body obj))))])
       (lambda ( ast port mode)
         (cond [(eq? (AST-output-syntax) 'concrete-syntax)
                (let ([recur (make-recur port mode)])
...
(struct Callq (target) #:transparent #:property prop:custom-print-quotable 'never
  #:methods gen:custom-write
  [(define write-proc
     (let ([csp (make-constructor-style-printer
                 (lambda (obj) 'Callq)
...
(define (parse-exp e)
  (match e
    ...
    [`(for ([,x ,seq]) ,body)
     (ForLoop x (parse-exp seq) (parse-exp body))]
    [`(while ,cnd ,body)
